    ...
    LaneCount<LANES>: SupportedLaneCount,
{
    ...
    pub(crate) fn update_centroid_distances(&self, state: &mut KMeansState<T>) {
        ...
        // manually calculate work-packet size, because rayon does not do static scheduling (which is more appropriate here)
        let work_packet_size =
            self.p_samples.len() / self.p_sample_dims / rayon::current_num_threads();
        self.p_samples
    ...
    ///     let kmean <f64, 8> = KMeans::new(samples, sample_cnt, sample_dims);
...
mod tests {
    ...
    fn calculate_cluster_assignments_multiplex(sample_dims: usize) {
        calculate_cluster_assignments::<f64, LANES>(sample_dims, 1e-10f64);
        calculate_cluster_assignments::<f32, LANES>(sample_dims, 1e-5f32);
    }
    ...
    fn distance_matrix_calculation_benchmark<T>(b: &mut Bencher)
    where
        T: SimdElement
            + Copy
            + Default
